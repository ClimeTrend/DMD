...
import pytest
import xarray as xr
from WeatherDMD.evaluate_wb2 import evaluate_wb2, _set_up_data_config, _set_up_eval_config
...
def test_set_up_data_config(temp_input_data):
    ...
    assert isinstance(data_config, config.Data), "data_config is not an instance of config.Data"
    assert data_config.paths.obs == obs_path, "Observation path is not set correctly"
    assert data_config.paths.forecast == forecast_path, "Forecast path is not set correctly"
    assert data_config.paths.output_dir == output_dir, "Output directory is not set correctly"
    assert data_config.selection.variables == variables, "Variables are not set correctly"
    assert data_config.selection.levels == levels, "Levels are not set correctly"
    assert data_config.selection.time_slice == slice(start_date, end_date), "Time slice is not set correctly"
...
def test_set_up_eval_config():
    ...
    assert isinstance(eval_config["spatial"], config.Eval), "spatial config is not an instance of config.Eval"
    assert isinstance(eval_config["non_spatial"], config.Eval), "non_spatial config is not an instance of config.Eval"
    assert "spatial_mse" in eval_config["spatial"].metrics, "spatial_mse metric is missing in spatial config"
    assert "rmse" in eval_config["non_spatial"].metrics, "rmse metric is missing in non_spatial config"
    assert isinstance(eval_config["spatial"].metrics["spatial_mse"], SpatialMSE), "spatial_mse metric is not an instance of SpatialMSE"
    assert isinstance(eval_config["non_spatial"].metrics["rmse"], RMSESqrtBeforeTimeAvg), "rmse metric is not an instance of RMSESqrtBeforeTimeAvg"
    assert "region1" in eval_config["spatial"].regions, "region1 is missing in spatial regions"
    assert "region2" in eval_config["spatial"].regions, "region2 is missing in spatial regions"
    assert isinstance(eval_config["spatial"].regions["region1"], SliceRegion), "region1 is not an instance of SliceRegion"
    assert isinstance(eval_config["spatial"].regions["region2"], SliceRegion), "region2 is not an instance of SliceRegion"

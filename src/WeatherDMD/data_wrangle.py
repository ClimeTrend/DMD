...
def get_days_before(data, year_0, day_0, T):
    ...
    get_days_before_ = lambda data, year_0, day_0, T: data[year_0][
        max(day_0 - T + 1, 0) : day_0 + 1
    ]
...
def get_days_after(data, year_0, day_0, T):
    ...
    get_days_after_ = lambda data, year_0, day_0, T: data[year_0][
        day_0 : min(day_0 + T, data[year_0].shape[0])
    ]
...
def window_mean(days_array, window, t=None):
    ...
    out = fftconvolve(
        days_array, np.ones((window, ny, nx)) / window, mode="valid", axes=0
    )
    out[out < 0] = 0.0

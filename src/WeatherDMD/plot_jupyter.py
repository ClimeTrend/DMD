...
def contour_data(data, x=None, y=None, mask=True):
    ...
    if x is None:
        x = onp.arange(data.shape[1])
    if y is None:
        y = onp.arange(data.shape[2])
    ...
    ind_x_widget = widgets.IntSlider(
        min=0,
        max=data.shape[2] - 1,
        step=1,
        continuous_update=True,
    ...
    ind_y_widget = widgets.IntSlider(
        min=0,
        max=data.shape[1] - 1,
        step=1,
        continuous_update=True,
...
def contour_compare(data1, data2, x=None, y=None, mask=True):
    ...
    def plot(i, BATCH, data2_BATCH, ind_x, ind_y, mask=True):
        ...
        ax[0, 0].axvline(x[ind_x], color="r", alpha=0.5)
        ax[0, 0].axhline(y[ind_y], color="k", alpha=0.5)
        ...
        # y-slice
        # ax[1].set(adjustable='box', aspect=1)
        ax[1, 0].plot(x, state_i[ind_y, :], color="k", alpha=0.5)
        ax[1, 0].plot(x, data2_state_i[ind_y, :], color="k", linestyle="-")
        ax[1, 0].set_xlim([x_min, x_max])

        # x-slice
        # ax[2].set(adjustable='box', aspect=1)
        ax[1, 1].plot(y, state_i[:, ind_x], color="r", alpha=0.5)
    ...
    if x is None:
        x = onp.arange(data1.shape[2])
    if y is None:
        y = onp.arange(data1.shape[1])
...
def plot_data1(data, x=None, fix_xlim=True, fix_ylim=False, mask=True):
    ...
    w = interactive(plotter, i=i_widget, data=fixed(data), mask=fixed(mask))
